...
export const fetchReservationTimeById = createAsyncThunk(
  ...
  async (id) => {
    try {
      const response = await fetch(`${process.env.API}/admin/reservation-times/${id}`);
      if (!response.ok) {
...
export const fetchReservationTimes = createAsyncThunk(
  ...
  async () => {
    try {
      const response = await fetch(`${process.env.API}/admin/reservation-times`);
      if (!response.ok) {
        throw new Error(`Failed to fetch reservation times: ${response.status}`);
      }
      return await response.json();
    } catch (error) {
...
export const createReservationTime = createAsyncThunk(
  ...
  async (timeData) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to create reservation time: ${response.status}`);
      }
...
export const updateReservationTime = createAsyncThunk(
  ...
  async ({ id, timeData }) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to update reservation time: ${response.status}`);
      }
      const data = await response.json();
...
export const deleteReservationTime = createAsyncThunk(
  ...
  async (id) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to delete reservation time: ${response.status}`);
      }
